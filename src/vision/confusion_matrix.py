...
def generate_confusion_data(
    ...
    ##########################################################################
    class_labels = dataset.get_classes().keys()
    preds_raw = []
    targets_raw = []
    for i in loader:
      if not cuda:
        preds_raw.append(model(i[0]))
      else:
        preds_raw.append(model(i[0].cuda()))
      targets_raw.append(i[1])
    for i in range(len(dataset)):
      preds[i] = torch.argmax(preds_raw[i//batch_size][i%batch_size])
      targets[i] = targets_raw[i//batch_size][i%batch_size]
...
def generate_confusion_matrix(
    ...
    if normalize:
        ...
        for i in range(confusion_matrix.shape[0]):
          # print(np.count_nonzero(target==i))
          confusion_matrix[i]/=(np.count_nonzero(targets==i))
